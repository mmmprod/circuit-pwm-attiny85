# CIRCUIT PWM FENÊTRE + µC — VERSION V1.14
...
+12V_PROT → R_LDO_IN (10Ω 0,5W ou 1W) → +12V_LDO_IN
+12V_LDO_IN → U1 pin3 (INPUT NCV2931AD-5.0R2G)
U1 pins 2,5 (GND) → GND
U1 pins 6,7,8 (OUTPUT) → +5V_MCU
+12V_LDO_IN → C_IN (100nF X7R 50V) → GND (proche U1)
+5V_MCU → C6 (10µF électro 25V) → GND
+5V_MCU → C7 (100nF X7R 50V) → GND
optionnel : 10µF 35V entre U1 Pin 3 et U1 Pin 2 (proche U1) 
U1 pin 4 : NC, Rien (en l'air)
...
Q2 pin3 (Drain) → R9 (100Ω déjà placée) → GATE_P
...
Code compatible: V1.7.8 (recommandé) ou V1.7.4+

## 6. Tests Obligatoires

Référence: Protocole_Test_PWM_V9_13.html
...
### V1.14 (Décembre 2025)
- Code V1.7.8: Protections défensives (timeout ADC, overflow VCC)
- Ultra-robuste contre défaillances hardware pathologiques
- Hardware identique à V1.10-V1.13

### V1.13 (Décembre 2025)
- Code V1.7.7: Détection ADC hors plage (défaut diviseur)
- Fail-safe si ADC < 50 ou > 950
- Hardware identique à V1.10/V1.11/V1.12

### V1.12 (Décembre 2025)
- Code V1.7.6: Dummy read stabilisation ADC après réveil
- Amélioration précision lectures ADC
- Hardware identique à V1.10/V1.11

### V1.11 (Décembre 2025)
- Code V1.7.5: ADC désactivé pendant sleep
- Consommation sleep réduite: ~0,65mA
- Hardware identique à V1.10
...
- ATtiny sleep (ADC off V1.7.8): 0,024mA
- **Total: ~0,65mA** (hors fenêtre, sortie OFF)

### Détection Défaut Hardware (V1.7.7+)
```
ADC < 50  → Défaut diviseur (court-circuit R2 vers GND)
ADC > 950 → Défaut diviseur (court-circuit R1 vers +5V)
→ Sortie OFF automatique (fail-safe)
→ Reteste toutes les ~1s (auto-recovery si transitoire)
```

### Protections Défensives V1.7.8
```
Timeout ADC readVCCmV():
  Si ADC hardware bloqué → timeout après ~1000 cycles → retourne 0
  Cas ultra-rare (ADC physiquement mort)

Protection overflow VCC:
  Si adc < 17 → VCC > 65V (impossible) → retourne 0
  Évite overflow silencieux uint32→uint16
```
